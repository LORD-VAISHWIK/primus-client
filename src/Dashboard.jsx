...
function StatsBar({ wallet, minutesLeft, currentUser }) {
  return (
    <div className="grid grid-cols-3 gap-4 mb-6">
      ...
      <div className="glass-card p-4">
        ...
        <div className="text-white text-xl font-semibold">{minutesLeft == null ? '—' : `${minutesLeft} min`}</div>
      </div>
      <div className="glass-card p-4">
        <div className="text-gray-400 text-xs">Signed in</div>
        <div className="text-white text-xl font-semibold">{currentUser?.username || 'User'}</div>
      </div>
    </div>
  );
}
...
function GameSelector({ games, selectedGame, onSelect, canLaunch, age, minutesLeft, activeSessionId, setActiveSessionId, pcId, currentUser, setWallet }) {
  return (
        ...
        {games.map((game) => (
          <div
            ...
            onClick={() => onSelect(game)}
             className={`relative cursor-pointer rounded-xl overflow-hidden transition-all duration-300 glass-card ${
  ...
  return (
    <div id="primus-chat">
      ...
      <div className="relative">
        <input value={text} onChange={e=>setText(e.target.value)} placeholder="Type a message" className="w-full pr-20 glass-input text-sm h-9" />
...
export default function Dashboard({ onLogout, onNavigate, currentUser, pcId, activePage, networkOnline }) {
  ...
  }, [pcId, activeSessionId, autoLbId, eventIds]);
    ...
    <div className="flex flex-col h-screen overflow-hidden">
      ...
      <div className="app-content flex-1 flex flex-col">
        <div className="px-8 py-4 space-y-4">
          <StatsBar wallet={wallet} minutesLeft={minutesLeft} currentUser={currentUser} />
          <QuickLaunch />
        </div>
